...
baseFilename = ['catmullRom_Ic'];


%% Create 20 base shapes
clear shapeParams
shapeParams(1,:) = [1 0.4 1 0.4 1 0.4 1 0.4 zeros(1,8)];
shapeParams(2,:) = [1 0.4 1 0.4 1 0.4 1 0.4 0.25 -0.25 0.25 -0.25 0.25 -0.25 0.25 -0.25];
shapeParams(3,:) = [1 0.6 1 0.6 1 0.6 1 0.6 zeros(1,8)];
shapeParams(4,:) = [0.25 0.3 1 0.3 0.25 0.3 1 0.3 zeros(1,8)];
shapeParams(5,:) = [0.2 0.9 1 0.9 0.2 0.9 1 0.9 zeros(1,8)];
fig(1) = figure(1); showPhenotypeBMP(reshape(shapeParams,[],16),d,fig(1))

%% Set shape variation parameters
numShapes = 10; scaling = [0.1 1.0]; rotation = [0 0.5*pi];
x = 1:numShapes; y = x; [X,Y] = ndgrid(x,y); 
selectedShapes{1} = [X(:),Y(:)];
des = ismember(selectedShapes{1}(:,1),[4,5,6,7,8])&ismember(selectedShapes{1}(:,2),[3,4,5,6,7,8]);
selectedShapes{2} = selectedShapes{1}(~des,:);
des = ismember(selectedShapes{1}(:,1),[4,5,6]);
selectedShapes{3} = selectedShapes{1}(~des,:);
des = ismember(selectedShapes{1}(:,1),[8,9,10]);
